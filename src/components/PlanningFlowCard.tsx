            ...
            {focusedMaterialIndex !== null && (
              <div className="fixed inset-0 z-[260] flex items-center justify-center bg-black/35 p-8" onClick={() => setFocusedMaterialIndex(null)}>
                ...
                <div className="relative flex h-[min(78vh,720px)] w-[min(1100px,90vw)] overflow-hidden rounded-2xl bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
                  <div className="relative flex flex-1 items-center justify-center bg-[#f7f8fa] p-8"><img src={documentImages[focusedMaterialIndex]} alt="文档素材预览" className="max-h-full max-w-full object-contain"/><button type="button" className="absolute left-4 top-1/2 -translate-y-1/2 rounded-full bg-white p-2 shadow hover:bg-gray-50" onClick={() => setFocusedMaterialIndex((focusedMaterialIndex - 1 + documentImages.length) % documentImages.length)}><ChevronLeft size={22}/></button><button type="button" className="absolute right-4 top-1/2 -translate-y-1/2 rounded-full bg-white p-2 shadow hover:bg-gray-50" onClick={() => setFocusedMaterialIndex((focusedMaterialIndex + 1) % documentImages.length)}><ChevronRight size={22}/></button></div>
