      ...
      {documentOpen && document && (
        <div className="fixed inset-0 z-[200] flex items-center justify-end bg-black/10 p-5" onClick={() => setDocumentOpen(false)}>
          ...
          <div className="relative flex items-center justify-center border-b pb-3 text-[16px] font-medium"><span>{document.name}</span><button className="absolute right-0" onClick={() => setDocumentOpen(false)}><X size={18}/></button></div>
            ...
            <div className="mt-4 text-[13px] font-medium text-gray-700">文档素材</div><div className="mt-2 columns-3 gap-2">{documentImages.map((image, index) => <div key={image} className="group relative mb-2 break-inside-avoid overflow-hidden rounded-[7px] bg-gray-100"><img src={image} className={`w-full object-cover transition-transform duration-300 group-hover:scale-[1.03] ${index % 3 === 0 ? 'h-28' : index % 3 === 1 ? 'h-20' : 'h-36'}`} alt="文档素材"/><div className="absolute inset-0 flex items-center justify-center gap-2 bg-black/0 opacity-0 transition-all group-hover:bg-black/25 group-hover:opacity-100"><button type="button" aria-label="放大" onClick={() => setFocusedMaterialIndex(index)} className="flex h-8 w-8 items-center justify-center rounded-full bg-white/95 text-gray-700 shadow-sm hover:bg-white"><Maximize2 size={14}/></button><button type="button" aria-label="添加到画布" onClick={() => onAddImage(image)} className="flex h-8 w-8 items-center justify-center rounded-full bg-white/95 text-gray-700 shadow-sm hover:bg-white"><Plus size={15}/></button><button type="button" aria-label="添加到对话" onClick={() => onAddToConversation({ id: `doc-material-${index}`, type: 'image', url: image, name: `文档素材${index + 1}` })} className="flex h-8 w-8 items-center justify-center rounded-full bg-white/95 text-gray-700 shadow-sm hover:bg-white"><MessageSquare size={14}/></button></div></div>)}</div>
