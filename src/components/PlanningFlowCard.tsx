        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-end bg-black/10 p-5" onClick={() => setDocumentOpen(false)}>
          ...
          <div className="relative flex items-center justify-center border-b pb-3 text-[16px] font-medium"><span>{document.name}</span><button className="absolute right-0" onClick={() => setDocumentOpen(false)}><X size={18}/></button></div>
            ...
            {focusedMaterialIndex !== null && (
              <div className="fixed inset-0 z-[260] flex items-center justify-center bg-white" onClick={() => setFocusedMaterialIndex(null)}>
                <button type="button" className="absolute right-7 top-6 z-10" onClick={() => setFocusedMaterialIndex(null)}><X size={24}/></button>
                <button type="button" className="absolute left-8 top-1/2 -translate-y-1/2 rounded-full p-3 hover:bg-gray-100" onClick={(event) => { event.stopPropagation(); setFocusedMaterialIndex((focusedMaterialIndex - 1 + documentImages.length) % documentImages.length); }}><ChevronLeft size={30}/></button>
                <img src={documentImages[focusedMaterialIndex]} alt="文档素材预览" className="max-h-[76vh] max-w-[62vw] object-contain" onClick={(event) => event.stopPropagation()} />
                <button type="button" className="absolute right-8 top-1/2 -translate-y-1/2 rounded-full p-3 hover:bg-gray-100" onClick={(event) => { event.stopPropagation(); setFocusedMaterialIndex((focusedMaterialIndex + 1) % documentImages.length); }}><ChevronRight size={30}/></button>
                <div className="absolute bottom-8 left-1/2 flex -translate-x-1/2 gap-2">{documentImages.map((image, index) => <button type="button" key={image} onClick={(event) => { event.stopPropagation(); setFocusedMaterialIndex(index); }} className={`h-14 w-16 overflow-hidden rounded border-2 ${index === focusedMaterialIndex ? 'border-black' : 'border-transparent'}`}><img src={image} className="h-full w-full object-cover" alt="缩略图" /></button>)}</div>
              </div>
            )}
          </div>
        </div>
