          ...
          <div className="relative flex items-center justify-center border-b pb-3 text-[16px] font-medium"><span>{document.name}</span><button className="absolute right-0" onClick={() => setDocumentOpen(false)}><X size={18}/></button></div>
            ...
            {focusedMaterialIndex !== null && (
              <div className="fixed inset-0 z-[260] flex items-center justify-center bg-black/35 p-8" onClick={() => setFocusedMaterialIndex(null)}>
                <button type="button" aria-label="关闭图片预览" className="fixed right-8 top-7 z-[270] flex h-9 w-9 items-center justify-center rounded-full bg-white text-gray-800 shadow-md transition hover:bg-gray-100" onClick={() => setFocusedMaterialIndex(null)}><X size={20}/></button>
                <div className="relative flex h-[min(78vh,720px)] w-[min(1100px,90vw)] overflow-hidden rounded-2xl bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
                  <div className="relative flex flex-1 items-center justify-center bg-[#f7f8fa] p-8 pb-24"><img src={documentImages[focusedMaterialIndex]} alt="文档素材预览" className="max-h-full max-w-full object-contain"/><button type="button" className="absolute left-4 top-1/2 -translate-y-1/2 rounded-full bg-white p-2 shadow hover:bg-gray-50" onClick={() => setFocusedMaterialIndex((focusedMaterialIndex - 1 + documentImages.length) % documentImages.length)}><ChevronLeft size={22}/></button><button type="button" className="absolute right-4 top-1/2 -translate-y-1/2 rounded-full bg-white p-2 shadow hover:bg-gray-50" onClick={() => setFocusedMaterialIndex((focusedMaterialIndex + 1) % documentImages.length)}><ChevronRight size={22}/></button><div className="absolute bottom-5 left-1/2 flex max-w-[80%] -translate-x-1/2 gap-2 overflow-x-auto rounded-xl bg-white/80 p-2 shadow-sm backdrop-blur">{documentImages.map((image, index) => <button type="button" key={image} onClick={() => setFocusedMaterialIndex(index)} className={`h-12 w-14 shrink-0 overflow-hidden rounded-md border-2 ${index === focusedMaterialIndex ? 'border-black' : 'border-transparent'}`}><img src={image} className="h-full w-full object-cover" alt={`缩略图 ${index + 1}`} /></button>)}</div></div>
                  <div className="relative flex w-[310px] shrink-0 flex-col justify-center border-l border-gray-100 p-7"><div className="text-lg font-semibold text-gray-900">文档素材 {focusedMaterialIndex + 1}</div><div className="mt-1 text-xs text-gray-400">图片 · {documentImages[focusedMaterialIndex].match(/(\d+)\/(\d+)$/)?.slice(1).join('×') || '260×180'} · {focusedMaterialIndex + 1} / {documentImages.length}</div><div className="mt-10 text-sm font-semibold text-gray-800">要点</div><div className="mt-2 text-sm leading-6 text-gray-600">本文为文档提取的视觉素材，可用于后续海报设计和画布创作。</div><div className="absolute bottom-8 right-7 flex gap-2"><button type="button" onClick={() => { onAddImage(documentImages[focusedMaterialIndex]); setFocusedMaterialIndex(null); }} className="rounded-full border px-5 py-2 text-sm text-gray-700">添加到画布</button><button type="button" onClick={() => { onAddToConversation({ id: `doc-preview-${focusedMaterialIndex}`, type: 'image', url: documentImages[focusedMaterialIndex], name: `文档素材${focusedMaterialIndex + 1}` }); setFocusedMaterialIndex(null); }} className="rounded-full bg-black px-5 py-2 text-sm text-white">添加到对话</button></div></div>
                </div>
              </div>
            )}
          </div>
